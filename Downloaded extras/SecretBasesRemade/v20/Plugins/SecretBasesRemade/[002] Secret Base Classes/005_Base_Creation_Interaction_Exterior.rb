module SecretBaseMethods
  ...
  def self.animate_base_opening(base_id, sprite, type_anim_id, appear_frame = -1)
    ...
    animation_frame_skip = Graphics.frame_rate / fr
    appear_time = 0
    if appear_frame > 0
      appear_time = appear_frame * animation_frame_skip
    end
    anim_length =  animation.frame_max * animation_frame_skip
    # set so that it will appear during the animation
    if appear_time>0
      pbWait(appear_time)
      $PokemonMap.current_base_id = base_id
    end
    pbWait(anim_length-appear_time)
    # just in case?
