module GameData
  class SecretBaseDecoration
    ...
    PBS_BASE_FILENAME = "secret_decorations"
    ...
    def name
      return pbGetMessageFromHash(MessageTypes::SECRET_BASE_DECORATIONS_NAMES, @real_name)
    end
    
    # @return [String] the translated description of this item
    def description
      return pbGetMessageFromHash(MessageTypes::SECRET_BASE_DECORATION_DESCRIPTIONS, @real_description)
    end
    ...
    def self.icon_filename(item)
      ...
      # Check for files
      ret = sprintf("Graphics/UI/SecretBases/Icons/%s", item_data.id)
