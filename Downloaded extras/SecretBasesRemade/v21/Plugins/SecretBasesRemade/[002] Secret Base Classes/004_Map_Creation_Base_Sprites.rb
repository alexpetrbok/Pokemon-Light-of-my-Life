class SecretBaseSprite
  ...
  def update_graphic
    ...
    if @active_base
      @sprite.setBitmap("Graphics/UI/SecretBases/tile_#{char_name}")
      @sprite.visible=true
    else
      filename="Graphics/UI/SecretBases/closed_#{char_name}"
...
module SecretBaseMethods
  ...
  def self.create_exterior_event(key_id, x, y, base_id)
    ...
    Compiler::push_move_route_and_wait(list, -1, [PBMoveRoute::THROUGH_ON,
                                                  PBMoveRoute::DOWN,
                                                  PBMoveRoute::THROUGH_OFF], 1)   # Move Route for player exiting warp
...
class Game_Map
  def reload_base_decorations
    return unless @map_id==SecretBaseSettings::SECRET_BASE_MAP
    # reset map data
    base_map = load_data(sprintf("Data/Map%03d.rxdata", @map_id))
    @map.data = base_map.data
    @events = {}
    # get base data
    base_id = $PokemonMap.current_base_id
