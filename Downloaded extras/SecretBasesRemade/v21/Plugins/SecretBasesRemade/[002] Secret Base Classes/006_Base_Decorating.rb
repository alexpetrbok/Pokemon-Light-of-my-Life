...
class Window_BaseDecorationsList < Window_DrawableCommand
  ...
  def initialize(bag, pocket, x, y, width, height)
    @bag = bag
    @pocket = pocket
    @itemUsing=AnimatedBitmap.new("Graphics/UI/SecretBases/secretBaseUsing")
    super(x, y, width, height)
  end
...
class PlaceDecoration_Scene
  def pbUpdate
    if ((System.uptime - @flicker_timer )>= 0.375) && @blink
      @flicker_timer = System.uptime
  ...
  def pbStartScene(bag,item,base)
    ...
    if @item
      ...
      @sprites["cursor"].bitmap = RPG::Cache.load_bitmap("Graphics/UI/SecretBases/", "secretBaseDeleter")
    ...
    @sprites["arrow"].setBitmap(sprintf("Graphics/UI/SecretBases/secretBaseArrow%d",$player.character_ID))
      ...
      if cant_place
        ...
        if pbConfirmMessage(_INTL("Place it here?")) { pbUpdate }
          ...
          pbFadeOutIn(99999){
            ...
            $game_map.reload_base_decorations
            $game_player.center($game_player.x, $game_player.y)
            pbMapInterpreter.get_self&.turn_left
            $game_temp.followers.map_transfer_followers
            Followers.hide_followers
            $scene.spriteset($game_map.map_id)&.dispose
            $scene.createSingleSpriteset($game_map.map_id)
          }
        end
      end
    else
