module Settings
  DEXNAV_ACTIVE = true
  DEXNAV_WINDOWSKIN = "Graphics/Windowskins/frlgtextskin"
end
...
class NewDexNav
  ...
  def initialize
    ...
    @sprites["locwindow"].setSkin(Settings::DEXNAV_WINDOWSKIN)
  ...
  def main
    ...
    loop do
      ...
      if Input.trigger?(Input::DOWN)
        ...
        if !$player.pokedex.seen?(@encarray[navMon])
  ...
  def main2
    if $currentDexSearch != nil
      ...
      itemRand = rand(3)
      navRand = rand(3)
      $game_variables[NavNums::Ability] = navRand
      ...
      case itemRand
      when 0
        $game_variables[NavNums::Item] = navItemCommon
      when 1
        $game_variables[NavNums::Item] = navItemUncommon
      when 2
        $game_variables[NavNums::Item] = navItemRare
      end
      navItem = $game_variables[NavNums::Item]
      ...
      @sprites["search"].setSkin(Settings::DEXNAV_WINDOWSKIN)
...
module NavNums
  ...
  Item = 401 #variable to set potential wild held items
end
...
EventHandlers.add(:on_wild_pokemon_created, :dexnav_chain,
  proc {|pkmn|
    ...
    if $currentDexSearch != nil
      ...
      encdata = encounter[GameData::EncounterType.get($game_temp.encounter_type).id]
      encdata = encdata.flatten
      if encdata.include?($currentDexSearch[0])
        ...
        itm = $game_variables[NavNums::Item]
        pokemon.item = itm == [] ? nil : itm[0]
        ...
        tempInt = $bag.has?(GameData::Item.get(:SHINYCHARM)) ? 256 : 768
