...
class VoltseonsPauseMenu_Scene
  ...
  def pbShowMenu
    ...
    duration.times do |i|
      ...
      @components.each_with_index do |component, i|
        next if component.is_a?(VPM_DateAndTimeHud)
        sprites = component.sprites
        cname = @component_names[i]
        sprites.each do |key, sprite|
          if sprite.y >= (Graphics.height / 2)
...
PluginManager.register({
  ...
  # Ensure this code runs after all other scripts are loaded
  class Scene_Map
    # Alias the start method to initialize the HUD
    alias vpm_scene_map_start start
    def start
      vpm_scene_map_start # Call the original start method
      # Initialize the Date and Time HUD
      @date_time_hud = VPM_DateAndTimeHud.new
      @date_time_hud.start_component(@viewport, self)
    end

    # Alias the update method to refresh the HUD
    alias vpm_scene_map_update update
    def update
      vpm_scene_map_update # Call the original update method
      # Update the Date and Time HUD
      @date_time_hud.update if @date_time_hud
    end

    # Alias the terminate method to dispose of the HUD
    alias vpm_scene_map_terminate terminate
    def terminate
      vpm_scene_map_terminate # Call the original terminate method
      # Dispose of the Date and Time HUD
      @date_time_hud.dispose if @date_time_hud
    end
  end
end
