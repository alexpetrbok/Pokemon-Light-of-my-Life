...
class BoxRanch
  ...
  def create_pokemon_event(pkmn, index, in_water = false)
    ...
    Compiler::push_script(event.pages[0].list, "show_chicken_interaction($box_ranch.pokemon_events[#{event.id}], #{event.id})")
...
def show_chicken_interaction(pkmn, event_id)
  # Step 1: Cry
  GameData::Species.play_cry(pkmn.species)

  chicken = get_chicken_by_pokemon(pkmn)


  # Step 2: Collect Egg
  pbCollectEggs(chicken)

  # Step 3: Daily Affection Bonus
  daily_interaction(chicken)
  ...
  loop do
    ...
    when 0  # Pet
      ...
      msg = case happiness
            when 0..49   then "#{pkmn.name} eyes you cautiously but accepts your hand."
            when 50..149 then "#{pkmn.name} nudges into your hand."
            when 150..249 then "#{pkmn.name} jumps into your lap and flaps happily."
            else "#{pkmn.name} practically glows with love for you!"
            end
      chicken.happiness += 2
      pbMessage(msg)
    when 1  # Play
      ...
      preferred_weather = chicken_data.weather_pref
      if (preferred_weather == current_weather)
        pbMessage("#{pkmn.name} runs around joyfully in this weather!")
        chicken.happiness += 2
      else
        pbMessage("#{pkmn.name} seems a little sluggish today...")
        chicken.happiness += 1
      end
    when 2  # Swap
      swap_chicken(chicken)
    else  # Exit or cancel
      break
    end
  end
