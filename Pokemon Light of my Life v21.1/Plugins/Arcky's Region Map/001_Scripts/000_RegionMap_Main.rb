...
class PokemonRegionMap_Scene
  ...
  def updateArrows
    ...
    @sprites["rightArrow"].visible = @spritesMap["map"].x > -1 * (@mapWidth - (Graphics.width - BEHIND_UI[1])) && !@previewBox.isExtShown
  end
    ...
    loop do
      ...
      if !@wallmap
        if (Input.trigger?(ARMSettings::SHOW_LOCATION_BUTTON) && @mode == 0 && ARMSettings::USE_LOCATION_PREVIEW) && getLocationInfo && !@previewBox.isShown
          ...
          @previewBox.showIt
          showPreviewBox
        elsif Input.trigger?(ARMSettings::SHOW_EXTENDED_BUTTON) && @previewBox.isShown && !@cannotExtPreview && @mode == 0
          pbPlayDecisionSE
          showExtendedPreview
        ...
        if Input.press?(ARMSettings::MOUSE_BUTTON_MOVE_MAP)
          if @previewBox.isShown || @previewBox.isUpdated
            ...
            if mousePos[0] < @oldPosX
              mox = -1 if @spritesMap["map"].x > -1 * (@mapWidth - (Graphics.width - BEHIND_UI[1]))
  ...
  def getDirectionInput(ox, oy, mox, moy)
    ...
    when 3, 6, 9
      ...
      mox = -1 if @spritesMap["map"].x > -1 * (@mapWidth - (Graphics.width - BEHIND_UI[1])) && ox == 0
    end
