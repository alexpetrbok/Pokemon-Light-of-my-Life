class PokemonRegionMap_Scene
  ...
  def extendedMain
    loop do
      ...
      if Input.trigger?(Input::BACK)
        ...
        drawDataMain
      elsif ARMSettings::PROGRESS_COUNT_POKEMON && Input.trigger?(ARMSettings::SHOW_EXTENDED_SUB_MENU) && @data[:wildAv]
  ...
  def changeExtBoxMainAndEncBox(type)
    return "ExtendedPreview/mapEncBox" if type == "Enc" && !ARMSettings::CHANGE_ENC_BOX_ON_ENCOUNTER_TYPE
  ...
  def getEncIcons
    ...
    @typeProgress = "- #{convertIntegerOrFloat(((@countSpecies.to_f / (@list.length * 3)) * 100).round(1))}%"
    ...
    @rowList[@pageIndex..(@pageIndex + (@colLength - 1))].each_with_index do |rows, j|
      ...
      rows.each_with_index do |species, i|
        ...
        if !seenFormAnyGender(mapID, species, form) && !caughtFormAnyGender(mapID, species, form)
          ...
          @encSprites[index].tone = Tone.new(0,0,0,255) if !caughtFormAnyGender(mapID, species, form)
        end
  ...
  def getSpeciesInfoPageTwo(species, speciesData, mapID)
    ...
    x = @rasterX + 250
    text << [totalDefeated.to_s, x, y, :center, @base, @shadow] if totalDefeated != defeated || (formsDefeated.is_a?(Numeric) && formsDefeated != 0)
    ...
    if totalCaught == 0 && totalDefeated == 0
      ...
      @sprites["TextRaster"].src_rect.height = 98
    end
